...
pub struct ConfirmSpec {
    ...
    /// Optional caller-selected evidence for this decision. Renderers may admit or decline it;
    /// they must never choose an image on the caller's behalf.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub decision_image: Option<DecisionImage>,
}
...
pub struct DecisionImage {
    pub path: String,
    #[serde(default)]
    pub required_for_decision: bool,
}
...
impl ConfirmSpec {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self
            .decision_image
            .as_ref()
            .is_some_and(|image| image.path.trim().is_empty())
        {
            return Err("confirm decision image path must not be empty".to_string());
        }
    ...
    pub fn into_request(
        ...
        Ok(ConfirmRequest {
            ...
            decision_image: self.decision_image,
            created_at_ms,
            expires_at_ms,
        })
    }
